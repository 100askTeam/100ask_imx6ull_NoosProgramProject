...
int  main()
{
	...
	if(c == '1' ) {
		...
		if (c == 'c') {
			...
			//sd_write_blocks(USDHC1, sd_read_buf, 2, 4);
		...
		//memset(sd_read_buf, 0x55, sizeof(sd_read_buf));
		//sd_write_blocks(USDHC2, sd_read_buf, 2, 4);
...
//#else
	...
	if(c == 'c') {
	...
	#if 0
		memset(sd_read_buf, 0, sizeof(sd_read_buf));
		sd_read_blocks(USDHC1, sd_read_buf, 0, 4);
		for(i = 0; i <2048; i++) {
			printf("%02x ", sd_read_buf[i]);
			if((i + 1)%16 == 0)
				printf("\r\n");
		...
		memset(sd_read_buf, 0xBB, sizeof(sd_read_buf));
		...
		printf("\r\n");
#endif
	//sd_write_blocks(USDHC1, led_imx_image, 0, sizeof(led_imx_image) / 512);
	//printf("write finish\r\n");
