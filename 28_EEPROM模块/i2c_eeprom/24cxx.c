...
{
	unsigned char temp;
	temp = i2c_read_one_byte(AT24CXX_ADDR, 255);	/* 避免每次开机都写AT24CXX */
	if(temp == 0X11)return 0;		   
	else {
		i2c_write_one_byte(AT24CXX_ADDR, 255, 0X11);
	    temp = i2c_read_one_byte(AT24CXX_ADDR, 255);	  
		if(temp == 0X11)return 0;
	}
	return 1;											  
}
...
{
	while(num--) {
		*arr++ = i2c_read_one_byte(AT24CXX_ADDR, addr++);
...
{
	while(num--) {
		i2c_write_one_byte(AT24CXX_ADDR, addr++, *arr);
